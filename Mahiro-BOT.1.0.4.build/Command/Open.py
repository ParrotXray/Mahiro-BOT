...
class Open(Cog_Extension):
    ...
    async def on_raw_reaction_add(self, payload: Any) -> None:
        ...
        ticket = await guild.create_text_channel(
            name=f"客服-{payload.member.name}", category=category, overwrites=overwrites
        )
        # 客服單關閉嵌入訊息
        embed = discord.Embed(title=f"{payload.member} 的客服單", description="請詳細說明您的問題並等待管理員處理",colour=payload.member.colour)
        ...
        async with aiofiles.open("database/close.json", "r") as file:  # 用read模式開啟資料檔案
            data = json.loads(await file.read())  # 讀取資料
        ...
        async with aiofiles.open("database/close.json", "w") as file:  # 用write模式開啟資料檔案
            await file.write(json.dumps(data, indent=4))  # 上傳更新後的資料
